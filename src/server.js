...
import { ensureStateFile, mergeDeck, readState, writeState } from "./storage.js";
    ...
    if (request.method === "POST" && url.pathname === "/api/rewrite") {
      ...
      if (!apiKey) {
        return sendJson(response, 500, {
          error: "Missing GROQ_API_KEY. Add it to your environment or .env file.",
        });
      }

      const rewrite = await rewriteSpanishDraft(text, apiKey);
      const currentState = await readState(STATE_FILE);
      const nextDeck = mergeDeck(currentState.deck, rewrite.cards);
      const nextState = await writeState(STATE_FILE, {
        note: rewrite.correctedText,
        deck: nextDeck,
        lastSavedAt: new Date().toISOString(),
        lastSummary: rewrite.changeSummary,
      });

      return sendJson(response, 200, {
        ...decorateState(nextState),
        newCards: rewrite.cards.filter((card) =>
          !currentState.deck.some((existing) => existing.id === `${card.sourceTerm.toLowerCase()}::${card.spanishTerm.toLowerCase()}`),
        ).length,
      });
    }
